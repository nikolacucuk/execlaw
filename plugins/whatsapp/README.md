...
Review decisions are persisted in the execlaw event log. Switching chats or
reloading the SPA no longer makes a previously sent or cancelled response
pending again. A completed decision exposes **Override decision**, which
returns that response to the pending state without changing its recipient.

The settings panel also includes **Show WhatsApp messages in a dedicated
WhatsApp chat**. When enabled, imported WhatsApp messages use a stable
dedicated WhatsApp scope. When disabled, the existing latest-active-chat
routing remains in effect.
