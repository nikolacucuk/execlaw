...
export interface AgentView { id:string; name:string; role_prompt:string; model:string|null; backend_purpose:string; tools:string[]; trust_policy:Record<string, unknown>; interval_secs:number; token_budget:number; max_runtime_secs:number; concurrency_limit:number; enabled:boolean; paused:boolean; next_run_at:number|null; last_run_at:number|null; last_run_status:string|null; last_error:string|null; }
...
export interface AgentRequest { name:string; role_prompt:string; model?:string|null; backend_purpose?:string; tools?:string[]; trust_policy?:Record<string, unknown>; interval_secs?:number; token_budget?:number; max_runtime_secs?:number; concurrency_limit?:number; enabled?:boolean; }
